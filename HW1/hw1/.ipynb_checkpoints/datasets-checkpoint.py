...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # ====== YOUR CODE: ======
        # Create a local seed for the creation of random instances based on the index of the image, insuring consistency
        random_state = np.random.RandomState(index)
        
        # Generate a random image
        random_image = torch.Tensor(random_state.randint(low=0, high=256, size=self.image_dim))
        random_label = random_state.randint(low=0, high=self.num_classes, size=1)
